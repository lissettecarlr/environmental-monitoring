...
namespace Helpers
{
    ...
    public sealed partial class MySqlHelper
    {
        ...
        /// <param name="ColumnsName">将要查询的列名</param>
        /// <returns></returns>
        private DataTable ReadRows(string connectionString,string sql,string ColumnsName)
        {
            DataSet dt = new DataSet();
            MySqlConnection mycon = new MySqlConnection(connectionString);
            using (MySqlDataAdapter adapter = new MySqlDataAdapter(sql, mycon))
            {
                adapter.Fill(dt, ColumnsName);
            }
            return dt.Tables[0];
        }
